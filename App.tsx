...
import { Home as HomeIcon, Calculator, Users, Wallet, PiggyBank, BarChart3, Settings as SettingsIcon, Info, Grid } from 'lucide-react';
import { Language, Theme, ExpenseEntry, SavingsGoal } from './types';
...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    ...
    { path: '/savings', icon: <PiggyBank size={20} />, label: t('savings') },
    { path: '/reports', icon: <BarChart3 size={20} />, label: t('reports') },
    { path: '/settings', icon: <SettingsIcon size={20} />, label: t('settings') },
  ];

  return (
    <div className={`min-h-screen flex flex-col ${theme === 'dark' ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-900'} transition-colors duration-300 overflow-x-hidden`} dir={isRtl ? 'rtl' : 'ltr'}>
      <header className={`fixed top-0 left-0 right-0 z-50 backdrop-blur-lg border-b ${theme === 'dark' ? 'bg-slate-900/90 border-slate-800' : 'bg-white/90 border-slate-200'}`}>
        <div className="max-w-md mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className={`text-base font-bold tracking-tight ${isRtl ? 'font-arabic' : ''}`}>{t('appName')}</h1>
          </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.path} to={item.path} className={`flex flex-col items-center justify-center w-full gap-0.5 transition-all duration-200 relative ${isActive ? 'text-emerald-500' : theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}>
                ...
                <span className={`text-[9px] font-bold uppercase tracking-tight ${isRtl ? 'font-arabic' : ''}`}>{item.label}</span>
...
const App: React.FC = () => {
  ...
  }, [lang, theme, currency, threshold, expenses, goals]);
  ...
  if (!isOnboarded) {
    return (
      <AppContext.Provider value={{ lang, setLang, theme, setTheme, currency, setCurrency, threshold, setThreshold, expenses, setExpenses, goals, setGoals, t, finishOnboarding }}>
        <WelcomePage />
      </AppContext.Provider>
    );
  }
